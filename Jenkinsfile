...
DESIRED_REPOSITORY = "https://github.com/GeographicaGS/Longitude-aiohttp-apispec.git"
PUBLISH_BRANCH = "master"
REPO_NAME = "longitude-aiohttp-apispec"
...
pipeline{
  ...
  stages {
    ...
    stage('Check-published') {
      ...
      steps {
        script {
          VERSION = sh(returnStdout: true, script: """grep -Po "^version = .*" pyproject.toml | sed 's/version\\ \\=\\ //g' | sed 's/\\"//g'""").trim()
        }
        sh "docker run --rm python:3.7.3-slim pip install geographica-longitude-aiohttp-apispec==${VERSION}"
      }
    }
  }
  post {
